...
; Date of completed work: 19/7/2019
;
; *******************************
...
start:
	...
	check_timer_3:
		...
		;******Indicate  button is currently pressed by six asterisks*******
		Part3:
			push r20
			push r21
			push r16
			...
			lds r20, ASTERISKS ; Print the * six times
			push r20
			rcall lcd_putchar
			pop r20
		...
		;******Indicate  button is currently pressed more than 200 milliseconds*******
		Part4:
			push r20
			push r21
			push r16
			...
			ldi ZH, high(DOTDASH_PATTERN) ; Making the Z points to the DOTDASH_PATTERN address
			ldi ZL, low(DOTDASH_PATTERN) 

			lds r16, BUTTON_COUNT+1 ; Checking if more than 10 button was pressed. If it is more than 10, stop printing
			ldi r17,10				
			cp r17,r16
			brcs skip111
			...
			lds r16, BUTTON_COUNT+1 ; If one botton pressed, Z is on the top of the DOTDASH_PATTERN
			cpi r16,1
			breq find_dotdash
			...
			; find next position  
			position_loop:         ; If more than one botton pressed, we need to find the next position we want the Z to point
				adiw Z,1
				dec r16
				cpi r16,1
				brne position_loop	
			...
			find_dotdash:
				lds r20,BUTTON_LENGTH  
				cpi r20,20       ; If BUTTON_LENGTH is more than 20, than it is a dash
				brcs is_dot		

			is_dash:
				ldi r20, '-'
				st Z,r20        ; load the dash to current Z position on DOTDASH_PATTERN
				rjmp print_result

			is_dot:
				ldi r20, '.'
				st Z,r20		; load the dot to current Z position on DOTDASH_PATTERN
				rjmp print_result

			print_result:		
				ldi r16, 0		; This is the row
				ldi r17, 0		; This is the column
				push r16
				push r17
				rcall lcd_gotoxy
				pop r17
			...
			ldi YH, high(DOTDASH_PATTERN) ; Making the Y points to the DOTDASH_PATTERN address
			ldi YL, low(DOTDASH_PATTERN)


			print_else:
				lds r21, BUTTON_COUNT+1  ; As button being pressed, print more symbol into the LED screen
					keep_11:
						ld r20,Y+		 ; Print everything in the DOTDASH_PATTERN address
						push r20
						rcall lcd_putchar
						pop r20
						dec r21
...
timer4:
	...
	skip:
		...
		do_nothing:
			;===============Part4=======================
			lds r22,BUTTON_LENGTH

			lds r23,BUTTON_CURRENT
			lds r21,BUTTON_PREVIOUS
			
			add r23,r21 ; Checking if it is an 0:0 condition, if it is ,do nothing
			tst r23
			breq skip1
			lds r23,BUTTON_CURRENT

			cp r21,r23
			brcs start_to_add ; When the 0:1 condition meet, starting to inc the BUTTON_COUNT

			cp r23,r21
			brcs end_to_add	  ; When the 1:0 condition meet, stoping to inc the BUTTON_COUNT

			still_add:
				inc r22
				sts BUTTON_LENGTH, r22  ; When the 1:1 condition meet, keeping to inc the BUTTON_COUNT
				rjmp skip1

			start_to_add:
				clr r22
				inc r22 ; Starting to add with 1 in	BUTTON_LENGTH, since 1:0 means pressed has began
				sts BUTTON_LENGTH, r22
				rjmp skip1
		...
		skip1:
			...
			; When BUTTON_CURRENT indicates a button is currently pressed, 
			; turn the lower-left portion of the display to '*', otherwise to ' '
			tst r21
			breq do_blank
				ldi r21,'*'
				sts ASTERISKS, r21
