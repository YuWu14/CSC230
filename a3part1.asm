...
; Date of completed work:15/7/2019
;
; *******************************
...
start:
	...
	check_timer_3:
		in temp, TIFR3
		sbrs temp, OCF3A
		rjmp blink_loop ; If timer3 did not reach the top, do the polling loop
		ldi temp, 1<<OCF3A
		out TIFR3, temp
		...
		pop r17
			...
			;******Checking the PULSE value which will be changed in timer1*******
			push r20
			lds r20, PULSE
			cpi r20,1
			breq swap_1; If timer1 handler is executed, swap the <> and blank
			pop r20
			; ====================================================================
...
timer1:
		...
		;****** When the timer’s interrupt handler is executed, heartbeat state kept in PLUSE data memory*******
		push r20		
		ldi r20, 1
		sts PULSE, r20
		pop r20
